...
NETWORK_MONITOR_SERVICE_FILE="/etc/systemd/system/network-monitor.service"
...
# Create a network-monitor service
echo "Creating network-monitor service..."
sudo bash -c "cat <<EOF > $NETWORK_MONITOR_SERVICE_FILE
[Unit]
Description=Network Monitor for Flask App
After=network.target
...
Group=www-data
WorkingDirectory=$APP_DIR
ExecStart=/usr/bin/python3 $APP_DIR/scripts/network_monitor.py
Restart=always
...
sudo systemctl enable heartfelt_echo network-monitor || handle_error "Failed to enable services"
sudo systemctl start heartfelt_echo network-monitor || handle_error "Failed to start services"
...
server {
    listen 80;
    server_name $WIFI_IP; # Replace with the Wi-Fi IP dynamically
